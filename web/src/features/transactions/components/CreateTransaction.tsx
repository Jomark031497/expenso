...
export const CreateTransaction = ({ userId, onClose, isOpen }: CreateTransactionProps) => {
  const {
    register,
    handleSubmit,
    reset,
    ...
    defaultValues: { userId, date: new Date(), amount: "0.00" },
  });

  const { data: wallets } = useWallets();

  const { isPending, mutate } = useMutation({
    mutationFn: async (values: NewTransaction) => await createTransaction({ ...values }),
    onSuccess: () => {
