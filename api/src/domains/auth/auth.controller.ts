...
export const signUpUserHandler = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    return res.appendHeader("Set-Cookie", lucia.createSessionCookie(data.session.id).serialize()).json(data.user);
  } catch (error) {
    return next(error);
  }
};
...
export const getAuthenticatedUserHandler = async (_req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!user) throw new AppError(401, "unauthorized. user not found in session");
...
export const logoutUserHandler = async (_req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!session) throw new AppError(401, "unauthorized. user not found in session");
